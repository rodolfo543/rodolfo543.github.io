# -*- coding: utf-8 -*-
"""
AXS Energia Unidade 08 - Debenture AXS811 - calculo PMT padrao ANBIMA com projecao Focus/BCB
Versao v2: ajuste da data efetiva da primeira integralizacao e exportacao XLSX.

Objetivo
- Calcular o fluxo da debenture AXS 08 sem imputar valores calculados pela Vortx.
- Usa IPCA oficial do SIDRA/IBGE quando disponivel e Focus/BCB para meses futuros.
- Regras principais da documentacao da emissao/aditivos:
  * Data de emissao: 01/07/2024.
  * Data de inicio da rentabilidade configurada como 10/07/2024, conforme primeira linha do historico de PUs da Vortx.
    Esta data e usada como data efetiva da primeira integralizacao, sem usar valores calculados pela Vortx.
  * Quantidade: 120.000 debentures.
  * PU inicial: R$ 1.000,00.
  * Remuneracao: IPCA + 11,00% a.a., base 252 dias uteis.
  * Juros pagos semestralmente em junho e dezembro.
  * Juros do periodo inicial, de DATA_INICIO_RENTABILIDADE ate 15/12/2024, incorporados ao saldo.
  * Amortizacao programada conforme cronograma do 4o Aditamento/AGD, em 33 parcelas ate 15/06/2041.

Como rodar
    python axs08_v2_focus_bcb_xlsx.py

Arquivos gerados
    controle_divida_axs08_v2_focus.csv
    controle_divida_axs08_v2_focus.xlsx
...
- Para conferir contra a Vortx, compare as colunas PU_VNa_Atualizado, PU_Juros, PU_Amort,
  Juros_R$, Amort_R$ e PMT_Total.
"""
...
# Ajuste aqui se a primeira integralizacao efetiva for diferente.
DATA_INICIO_RENTABILIDADE = date(2024, 7, 10)
DATA_INCORPORACAO_JUROS_INICIAL = date(2024, 12, 15)
TAXA_AA = Decimal("0.11")
QUANTIDADE = Decimal("120000")
PU_INICIAL = Decimal("1000.00000000")

# Cronograma vigente conforme AGD/4o Aditamento: Data, TAI (% do saldo VNA a amortizar).
# A coluna "Amortizacao" do anexo e derivada da TAI sobre o saldo remanescente;
# para o calculo da parcela usa-se a TAI aplicada ao saldo VNA atualizado na data.
CRONOGRAMA_RAW = [
    ("2025-06-15", "0.0991"), ("2025-12-15", "5.3000"),
    ("2026-06-15", "2.0000"), ("2026-12-15", "0.4818"),
    ("2027-06-15", "2.5000"), ("2027-12-15", "2.0000"),
    ("2028-06-15", "2.9000"), ("2028-12-15", "2.3000"),
    ("2029-06-15", "3.3000"), ("2029-12-15", "2.8000"),
    ("2030-06-15", "3.8000"), ("2030-12-15", "3.5000"),
    ("2031-06-15", "4.4704"), ("2031-12-15", "4.3000"),
    ("2032-06-15", "4.5000"), ("2032-12-15", "4.2000"),
    ("2033-06-15", "5.1000"), ("2033-12-15", "5.1000"),
    ("2034-06-15", "6.1000"), ("2034-12-15", "6.3000"),
    ("2035-06-15", "7.2000"), ("2035-12-15", "7.2000"),
    ("2036-06-15", "8.0000"), ("2036-12-15", "8.2906"),
    ("2037-06-15", "9.0401"), ("2037-12-15", "10.7031"),
    ("2038-06-15", "11.9859"), ("2038-12-15", "15.5637"),
    ("2039-06-15", "16.5000"), ("2039-12-15", "24.0101"),
    ("2040-06-15", "30.0000"), ("2040-12-15", "50.2743"),
    ("2041-06-15", "100.0000"),
]
...
def calcular_fluxo() -> Tuple[List[Dict[str, object]], str]:
    ...
    # Evento especial aprovado no 3o aditamento: juros iniciais incorporados em 15/12/2024.
    saldo_pu, detalhes_ipca, data_ipca_atual = aplicar_ipca_ate(
        saldo_pu,
        data_ipca_atual,
        DATA_INCORPORACAO_JUROS_INICIAL,
        indices,
        fonte_mes,
    )
    du_incorp = dias_uteis(data_ref_juros, DATA_INCORPORACAO_JUROS_INICIAL)
    ...
    linhas.append({
        "Evento": "Incorporacao Juros Inicial",
        "Data_Ref": DATA_INCORPORACAO_JUROS_INICIAL.strftime("%d/%m/%Y"),
        "Data_Pgto": "",
        "DU_Juros": du_incorp,
        ...
        "TAI_Amort": Decimal("0"),
        "PU_VNa_Ini": trunc_dec(PU_INICIAL, 8),
    ...
    data_ref_juros = DATA_INCORPORACAO_JUROS_INICIAL
...
def salvar_xlsx(linhas: List[Dict[str, object]], caminho: str, fonte_ipca: str) -> None:
    ...
    tab = Table(displayName="TabelaControleAXS08", ref=ref)
    ...
    for cell in ws[1]:
        ...
        if cell.value == "PU_Juros_Incorporado":
            cell.comment = Comment("Juros iniciais incorporados ao VNA em 15/12/2024, conforme 3o aditamento.", "ChatGPT")

    ws2 = wb.create_sheet("Parametros")
    params = [
        ["Campo", "Valor"],
        ["Emissao", "AXS Energia Unidade 08 - AXS811"],
        ["Data de Emissao", DATA_EMISSAO.strftime("%d/%m/%Y")],
        ["Data de Inicio da Rentabilidade", DATA_INICIO_RENTABILIDADE.strftime("%d/%m/%Y")],
        ["Data de Incorporacao dos Juros Iniciais", DATA_INCORPORACAO_JUROS_INICIAL.strftime("%d/%m/%Y")],
        ["Taxa AA", float(TAXA_AA)],
        ["Quantidade", float(QUANTIDADE)],
        ["PU Inicial", float(PU_INICIAL)],
        ["Fonte IPCA", fonte_ipca],
        ["Observacao", "Calculo sem imputar valores calculados pela Vortx; historico de PU usado apenas para identificar a data inicial quando informado."],
    ]
    for r in params:
        ws2.append(r)
    for cell in ws2[1]:
...
def main() -> None:
    ...
    arquivo_csv = "controle_divida_axs08_v2_focus.csv"
    arquivo_xlsx = "controle_divida_axs08_v2_focus.xlsx"
    ...
    imprimir_linha("15/06/2025", linhas)
    imprimir_linha("15/12/2025", linhas)
    imprimir_linha("15/06/2041", linhas)
